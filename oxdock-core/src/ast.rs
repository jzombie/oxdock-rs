...
pub fn parse_script(input: &str) -> Result<Vec<Step>> {
	...
	while let Some((line_no, rawline)) = queue.pop_front() {
		...
		let (groups, mut remainder_opt) = if let Some(rest) = line.strip_prefix('[') {
			let end = rest
				.find(']')
		...
		let rest_owned = parts.next().map(|s| s.trim().to_string()).unwrap_or_default();
		let cmd = Command::parse(op_owned.as_str())
			.ok_or_else(|| anyhow::anyhow!("line {}: unknown instruction '{}'", line_no, op_owned))?;
		...
		let kind = match cmd {
			...
			Command::Cat => {
				let path = remainder.split_whitespace().next().filter(|s| !s.is_empty()).ok_or_else(|| {
					anyhow::anyhow!("line {}: CAT requires <path>", line_no)
				})?;
