...
#[cfg(not(miri))]
impl Backend {
    pub(super) fn new(_root: &GuardedPath, _build: &GuardedPath) -> Result<Self> {
        Ok(Self {})
    }

    pub(super) fn create_dir_all_abs(
        &self,
        root: &GuardedPath,
        path: &GuardedPath,
    ) -> Result<()> {
    ...
    pub(super) fn read_file(&self, path: &GuardedPath) -> Result<Vec<u8>> {
        let data =
            fs::read(path.as_path()).with_context(|| format!("failed to read {}", path.display()))?;
        Ok(data)
    }
...
fn seed_from_host(root: &GuardedPath, state: &std::rc::Rc<std::cell::RefCell<SyntheticRootState>>) -> Result<()> {
    ...
    fn walk(dir: &Path, rel_prefix: &str, state: &mut SyntheticRootState) {
