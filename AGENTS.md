...
- **Edit the templates instead**: per-crate sources live under `<crate>/.oxdock/template/` (`header.tmpl`, `fragments/*.md` verbatim or `fragments/*.tmpl` expanded, `footer.tmpl`, declared by `target.json` with its `out` path); the workspace `README.md` comes from `.oxdock/template/readme/` (`output.tmpl` master template plus `fragments/`). Shared strings stay single-sourced in `.oxdock/template/_global/values.json` and are referenced as `{{ $docs_global.* }}`. Re-run `docs-gen` after changing templates and commit the regenerated outputs.
