...
- **Do not edit `README.md` files by hand.** All READMEs are generated by the `crates/docs-gen/` crate (`cargo run -p docs-gen`); any manual additions will be overwritten on the next run. See `crates/docs-gen/README.md` for the pipeline, config (`docs-gen.json`), and stage kinds.
- **Edit the templates instead**: per-crate sources live under `<crate>/.oxdock/template/` (`header.md.tmpl`, `fragments/*.md` verbatim or `fragments/*.md.tmpl` expanded, `footer.md.tmpl`, declared by `target.json` with its `out` path); the workspace `README.md` comes from `.oxdock/template/readme/` (`output.md.tmpl` master template plus `fragments/`). The engine expands any `*.tmpl` suffix and stays format neutral; this repo standardizes on `*.md.tmpl` for markdown so the output type is visible. Shared strings stay single-sourced in `.oxdock/template/_global/values.json` and are referenced as `{{ $docs_global.* }}`. Shared lead copy lives once in `.oxdock/template/shared/intro.md.tmpl` and is referenced by both `output.md.tmpl` files. Re-run `docs-gen` after changing templates and commit the regenerated outputs.
- **Prose style**: do not use em dashes, en dashes, or hyphens as punctuation in README prose and templates. Use periods or colons instead. Hyphens inside code, crate names, CLI flags, and versions remain allowed. Fragments that contain DSL `{{ ... }}` examples must stay `.md` verbatim so the placeholders survive; only `*.md.tmpl` files expand `{{ env:CRATE_VERSION }}`.
